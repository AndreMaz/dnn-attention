...
def generateDataset(num_samples, sample_length, min_value, max_value, SOS_CODE, EOS_CODE, vocab_size):
    ...
    for _ in range(num_samples):
        ...
        encoderInputs.append(tf.convert_to_tensor(
            enc_input_list, dtype="float32"))

        decoderInputs.append(tf.convert_to_tensor(
            dec_input_list, dtype="float32"))

        # Decoder's output
        dec_out_enc = np.zeros(
            (sample_length + 1, sample_length + 1), dtype='int32')
